...
{
    public class AssetLoadManager : SingletonMonoBehaviour<AssetLoadManager>
    {
        ...
        {
            foreach (var loadedAsset in m_AsyncAddressableAssets)
            {
            ...
            m_AsyncAddressableAssets.Clear();

            foreach (var loadedAsset in m_SyncAddressableAssets)
            {
                if (loadedAsset.Value != null)
                {
                    Addressables.Release(loadedAsset.Value);
        ...
        public ScriptableObject LoadScriptableObject(string key)
        {
            return Load<ScriptableObject>(key);
        }

        private T Load<T>(string key) where T : UnityEngine.Object
        {
            if (string.IsNullOrEmpty(key))
            {
                DebugLogWrapper.LogError($"Invalid key name {key}");
